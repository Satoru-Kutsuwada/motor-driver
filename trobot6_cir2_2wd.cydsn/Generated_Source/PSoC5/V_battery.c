...
{
    uint8 staticBits = (V_battery_DR & (uint8)(~V_battery_MASK));
    V_battery_DR = staticBits | ((uint8)(value << V_battery_SHIFT) & V_battery_MASK);
}
...
{
	CyPins_SetPinDriveMode(V_battery_0, mode);
}
...
{
    return (V_battery_PS & V_battery_MASK) >> V_battery_SHIFT;
}
...
{
    return (V_battery_DR & V_battery_MASK) >> V_battery_SHIFT;
}


/* If interrupt is connected for this Pins component */ 
#if defined(V_battery_INTSTAT) 
    ...
    *******************************************************************************/
    ...
    {
		if((position & V_battery_0_INTR) != 0u) 
		{ 
			 V_battery_0_INTTYPE_REG = (uint8)mode; 
		}
    }
    ...
    *******************************************************************************/
    ...
    {
        return (V_battery_INTSTAT & V_battery_MASK) >> V_battery_SHIFT;
    }

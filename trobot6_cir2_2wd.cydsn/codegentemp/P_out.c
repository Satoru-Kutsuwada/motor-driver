...
void P_out_SetDriveMode(uint8 mode)
{
	...
	CyPins_SetPinDriveMode(P_out_1, mode);
	CyPins_SetPinDriveMode(P_out_2, mode);
	CyPins_SetPinDriveMode(P_out_3, mode);
}
...
/* If interrupt is connected for this Pins component */ 
    ...
    *******************************************************************************/
    void P_out_SetInterruptMode(uint16 position, uint16 mode)
    {
		...
		if((position & P_out_1_INTR) != 0u) 
		{ 
			 P_out_1_INTTYPE_REG = (uint8)mode; 
		} 
		if((position & P_out_2_INTR) != 0u) 
		{ 
			 P_out_2_INTTYPE_REG = (uint8)mode; 
		} 
		if((position & P_out_3_INTR) != 0u) 
		{ 
			 P_out_3_INTTYPE_REG = (uint8)mode; 
		}
    }
